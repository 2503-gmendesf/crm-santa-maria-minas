        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" id="form-lead-grid">
          ...
          ${isMaster() ? `<div class="form-group">
            ...
            <select class="form-control" id="fl-resp-id">
              ${USUARIOS.filter(u=>u.tipo==='auxiliar').map(u=>`<option value="${u.id}" ${(lead?.responsavel_id)===u.id?'selected':''}>${u.nome}</option>`).join('')}
            </select>
          </div>` : ''}
...
async function salvarLead(editId, btn) {
  ...
  const respId       = isMaster() ? document.getElementById('fl-resp-id')?.value : currentUser.id;
