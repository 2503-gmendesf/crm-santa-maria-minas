...
function getLeadsFiltrados() {
  ...
  if (leadsFiltros.serie)       leads = leads.filter(l => (Array.isArray(l.serie_interesse) ? l.serie_interesse : [l.serie_interesse]).includes(leadsFiltros.serie));
...
function renderLeadRow(l) {
  ...
  const waMensagem = encodeURIComponent(`Olá ${l.nome_responsavel}! Sou ${currentUser.nome} da Rede de Colégios Santa Maria. Gostaria de conversar sobre a matrícula de ${l.nome_aluno} para ${(Array.isArray(l.serie_interesse)?l.serie_interesse.join(", "):l.serie_interesse)}.`);
  ...
  return `<tr onclick="abrirDetalhe('${l.id}')" style="${vencida?'background:#fff9f9':''}">
    ...
    <td>${(Array.isArray(l.serie_interesse)?l.serie_interesse:[l.serie_interesse]).map(s=>`<span class="badge badge-neutral" style="font-size:11px">${s}</span>`).join(" ")}</td>
...
function buildDrawerContent(lead) {
  ...
  const waMensagem = encodeURIComponent(`Olá ${lead.nome_responsavel}! Sou ${currentUser.nome} da Rede de Colégios Santa Maria. Gostaria de conversar sobre a matrícula de ${lead.nome_aluno} para ${(Array.isArray(lead.serie_interesse)?lead.serie_interesse.join(", "):lead.serie_interesse)}.`);
  ...
  return `
    ...
    <div class="drawer-body">
      ...
      <div class="detail-section">
        ...
        <div class="detail-row"><span class="detail-row__label">Série / Turno</span><span class="detail-row__value">${(Array.isArray(lead.serie_interesse)?lead.serie_interesse:[lead.serie_interesse]).join(", ")} · ${lead.turno_interesse}</span></div>
        ...
        <div class="detail-row"><span class="detail-row__label">Primeiro Contato</span><span class="detail-row__value">${lead.data_primeiro_contato ? formatDate(lead.data_primeiro_contato) : '—'}</span></div>
      ...
      <div class="modal-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" id="form-lead-grid">
          ...
          <div class="form-group" style="grid-column:1/-1">
            <label class="form-label">Série(s) de Interesse <span class="required">*</span></label>
            <div id="fl-series-wrap" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px">
              ${SERIES.map(s=>{
                const selecionado = Array.isArray(lead?.serie_interesse)
                  ? lead.serie_interesse.includes(s)
                  : lead?.serie_interesse === s;
                return `<label style="display:flex;align-items:center;gap:4px;padding:5px 10px;border:1.5px solid var(--border);border-radius:20px;cursor:pointer;font-size:13px;user-select:none;transition:all .15s"
                  class="serie-chip${selecionado?' serie-chip--on':''}">
                  <input type="checkbox" name="fl-serie" value="${s}" ${selecionado?'checked':''} style="display:none"
                    onchange="this.closest('label').classList.toggle('serie-chip--on',this.checked)">
                  ${s}
                </label>`;
              }).join('')}
            </div>
        ...
        <div style="margin-top:16px;padding:12px;background:var(--primary-pale);border-radius:var(--radius-md)">
          <div style="font-weight:600;font-size:13px;margin-bottom:10px">📅 Datas</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
            <div class="form-group">
              <label class="form-label">Data do Primeiro Contato</label>
              <input type="date" class="form-control" id="fl-primeiro-contato"
                value="${lead?.data_primeiro_contato ? lead.data_primeiro_contato.slice(0,10) : new Date().toISOString().slice(0,10)}">
            </div>
            <div class="form-group">
          ...
          <div class="form-group">
            <label class="form-label">Descrição da Próxima Ação</label>
...
async function salvarLead(editId, btn) {
  ...
  const serie = [...document.querySelectorAll('input[name="fl-serie"]:checked')].map(c=>c.value);
  const unidade     = parseInt(document.getElementById('fl-unidade')?.value);
  const origem      = document.getElementById('fl-origem')?.value;

  if (!responsavel || !aluno || !telefone || !serie.length || !unidade || !origem) {
  ...
  const primContato       = document.getElementById('fl-primeiro-contato')?.value;
  ...
  if (editId) {
    ...
    lead.data_proxima_acao     = proxData ? new Date(proxData).toISOString() : null;
    lead.data_primeiro_contato = primContato ? new Date(primContato).toISOString() : lead.data_primeiro_contato || null;
    ...
    const novoLead = {
      ...
      origem,
      data_criacao: agora,
      data_primeiro_contato: primContato ? new Date(primContato).toISOString() : agora,
